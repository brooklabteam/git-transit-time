...
phylo_dat <- read.csv(file = paste0(homewd, "/data/revision-3/phylo_names_tomatch.csv"), header = T, stringsAsFactors = F )
#View(dat)
...
dat.sum.tot <- merge(
  dat.sum.tot,
  phylo_dat[, c("genus.species", "phylo_name")],
  by = "genus.species",
  all.x = TRUE          # keep every row of dat.sum.tot2; unmatched get NA
)

#dat.sum.tot <- dat.sum.tot %>% filter(!is.na(phylo_name) & phylo_name != "")
