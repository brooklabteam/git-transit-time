...
#dat <- read.csv(file = paste0(homewd, "/data/RuhsMcFerrin_database_R2_updated_diet.csv"), header = T, stringsAsFactors = F )
dat <- read.csv(file = paste0(homewd, "/data/revision-1/RuhsMcFerrin_database_R2_updated_diet.csv"), header = T, stringsAsFactors = F )
#View(dat)
...
#remove those that have an unknown trial diet
...
subset(dat.sum.tot, !is.na(transit)) #138
...
write.csv(dat.sum.tot, "data/revision-1/output/dat_sum_tot_GIT_R2.csv")
...
unique(dat.sum.tot$re_class)
# [1] "Artiodactyla"  "Carnivora"     "Primates"      "Rodentia"      "Reptilia"      "Flying Birds"  "Chiroptera" 
...
write.csv(dat.sum.tot, "data/revision-1/output/dat_sum_tot_clean_GIT_R2.csv") #I don't know why there are some NA so manually fixing and re-uploading
...
#subset(dat.sum.tot, !is.na(transit)) #125
subset(dat.sum.tot2, !is.na(total_MRT)) #79
...
write.csv(dat.sum.tot2, "data/dat_sum_tot_MRT_R2.csv")
...
unique(dat.sum.tot2$re_class)
#"Non-Flying Birds" "Artiodactyla"     "Primates"         "Rodentia"         "Chiroptera" 
...
write.csv(dat.sum.tot2, "data/dat_sum_tot_clean_MRT_R2.csv") #I don't know why there are some NA so manually fixing and re-uploading
# dat.sum.tot_clean <- read.csv(file = paste0(homewd, "data/dat.sum.tot.csv"), header = T, stringsAsFactors = F )
#dat.sum.tot_clean <- dat.sum.tot
