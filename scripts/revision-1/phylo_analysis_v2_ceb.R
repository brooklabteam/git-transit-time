...
#converges fine with 'transit_hrs' but not log_transit_hrs
pgls_model_GIT_0 <- gls(log_transit_hrs ~ re_class, 
                        data = GIT_pruned, 
...
AIC(pgls_model_GIT_0, pgls_model_GIT_1) #no phylogeny is better performing

#check that residuals are normal
hist(resid(pgls_model_GIT_1), main = "Residual Histogram", xlab = "Residuals")
hist(resid(pgls_model_GIT_0), main = "Residual Histogram", xlab = "Residuals")
...
AIC(pgls_model_GIT_1, pgls_model_GIT_1_reduced, pgls_model_GIT_0, pgls_model_GIT_0_reduced)
# df      AIC
...
#what if we tested adding a flyer class instead?
pgls_model_GIT_1_reduced_flyer <- gls(log_transit_hrs ~ flyer, 
                                data = GIT_pruned, 
                                correlation = cor_phylo_fixed1,
                                method = "ML")

pgls_model_GIT_0_reduced_flyer <- gls(log_transit_hrs ~ flyer, 
                                      data = GIT_pruned, 
                                      correlation = cor_phylo_fixed0,
                                      method = "ML")

AIC(pgls_model_GIT_1_reduced_flyer, pgls_model_GIT_0_reduced_flyer) 
#phylogenetic model is best. 
#"flyer" is HIGHLY significant without the phylogenetic effects but also still significant with them

#try with the weights as well


pgls_model_GIT_1_flyer <- gls(log_transit_hrs ~ flyer, 
                              data = GIT_pruned, 
                              correlation = cor_phylo_fixed1,
                              method = "ML",
                              weights = varIdent(form = ~1|typical.diet))

pgls_model_GIT_0_flyer <- gls(log_transit_hrs ~ flyer, 
                              data = GIT_pruned, 
                              correlation = cor_phylo_fixed0,
                              method = "ML",
                              weights = varIdent(form = ~1|typical.diet))

AIC(pgls_model_GIT_1_flyer, pgls_model_GIT_0_flyer, pgls_model_GIT_1_reduced_flyer, pgls_model_GIT_0_reduced_flyer) 

#full phylogenetic model with phylogeny is best
summary(pgls_model_GIT_1_flyer) #flyer is highly sign even with phylogenetic effects
...
pgls_model_GIT_20 <- gls(log_transit_hrs ~ log10(mass_kg):re_class, 
                        data = GIT_pruned, 
                        correlation = cor_phylo_fixed0,
                        method = "ML",
                        weights = varIdent(form = ~1|typical.diet))

AIC(pgls_model_GIT_20, pgls_model_GIT_2) #better with phylogeny
...
#try with flyer
pgls_model_GIT_2_flyer <- gls(log_transit_hrs ~ log10(mass_kg)*flyer, 
                        data = GIT_pruned, 
                        correlation = cor_phylo_fixed1,
                        method = "ML",
                        weights = varIdent(form = ~1|typical.diet))

pgls_model_GIT_20_flyer <- gls(log_transit_hrs ~ log10(mass_kg)*flyer, 
                         data = GIT_pruned, 
                         correlation = cor_phylo_fixed0,
                         method = "ML",
                         weights = varIdent(form = ~1|typical.diet))

AIC(pgls_model_GIT_20_flyer, pgls_model_GIT_2_flyer) #better with phylogeny by a lot

summary(pgls_model_GIT_2_flyer) 
#flyer neg sig effect.
#mass pos weak effect.
#mass and flyer positive but not sig

#try just with additive effects
pgls_model_GIT_2_flyer <- gls(log_transit_hrs ~ log10(mass_kg)+flyer, 
                              data = GIT_pruned, 
                              correlation = cor_phylo_fixed1,
                              method = "ML",
...
summary(pgls_model_GIT_2_flyer) 
#flyer neg sig interaction effect
#mass positive sig interaction
